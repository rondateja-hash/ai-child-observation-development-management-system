...
import { User, Child, getChildAvatar, getUserAvatar } from "../types";
import { motion } from "motion/react";
import { Sparkles, UserCheck, ShieldAlert, ArrowRight, Loader2, Users, Baby } from "lucide-react";

interface ProfileSelectorProps {
  user: User;
  onProfileSwitched: (user: User) => void;
  onCancel?: () => void;
  addToast: (title: string, message: string, type: 'success' | 'error' | 'warning' | 'info') => void;
}

export default function ProfileSelectorView({ user, onProfileSwitched, onCancel, addToast }: ProfileSelectorProps) {
  ...
  const handleSelectTeacher = async (profileId: string) => {
    try {
      ...
      addToast(
        "Profile Switched",
        `Acting as ${response.user.name} (${response.user.role})`,
        "success"
      );
  ...
  const handleSelectChild = async (child: Child) => {
    try {
      ...
      const matchingParent = parents.find(
        (p) => p.name.toLowerCase().trim() === child.parentName.toLowerCase().trim()
      );
      ...
      addToast(
        "Welcome",
        `Logged in as ${response.user.name}. Accessing reports for ${child.fullName}.`,
        "success"
      );
  ...
  const getTeacherAvatar = (teacherName: string) => {
    if (teacherName.includes("Priya")) return "https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=150";
    if (teacherName.includes("Sneha")) return "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=150";
      ...
      <div className="max-w-4xl w-full bg-white rounded-3xl border border-slate-200 shadow-2xl p-6 sm:p-10 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-6">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3 text-center bg-red-50/50 rounded-2xl border border-red-100 p-6">
            ...
            <div className="space-y-6">
                  ...
                  {teachers.map((t) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="p-5 border border-slate-200 hover:border-blue-400 bg-white rounded-2xl transition-all shadow-sm hover:shadow-md flex items-start gap-4 cursor-pointer relative group"
                        onClick={() => !switching && handleSelectTeacher(t.id)}
                      >
                        <img
                          src={getTeacherAvatar(t.name)}
                          alt={t.name}
                          className="h-14 w-14 rounded-xl object-cover bg-slate-100 border border-slate-200 shrink-0"
                        ...
                        <div className="self-center bg-slate-50 group-hover:bg-blue-600 group-hover:text-white p-2 rounded-xl transition-all">
                          {isCurrentSwitched ? (
                            <Loader2 className="h-4 w-4 animate-spin text-blue-600 group-hover:text-white" />
                          ) : (
                  ...
                  {children.map((child) => {
                    ...
                    return (
                      <motion.div
                        key={child.id}
                        whileHover={{ scale: 1.01, y: -2 }}
                        className="p-5 border border-slate-200 hover:border-blue-500 bg-white rounded-2xl transition-all shadow-sm hover:shadow-md flex flex-col justify-between gap-4 cursor-pointer relative group"
                        onClick={() => !switching && handleSelectChild(child)}
                      >
              ...
              {onCancel && (
                <div className="pt-4 flex justify-center">
                  <button
                    onClick={onCancel}
                    className="px-6 py-2 border border-slate-200 hover:bg-slate-50 text-slate-600 font-semibold rounded-xl text-sm transition-all cursor-pointer"
                  >
                    Go Back
                  </button>
                </div>
