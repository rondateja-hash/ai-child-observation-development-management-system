...
import { Users, BookOpen, Sparkles, Clock, Calendar, CheckSquare, ShieldCheck, ArrowRight, TrendingUp } from "lucide-react";
...
export default function DashboardView({ user, addToast, setActiveTab }: DashboardProps) {
  ...
  return (
    <div className="space-y-8 font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <motion.div
          whileHover={{ y: -4 }}
          className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden group cursor-pointer"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm lg:col-span-2">
          ...
          <div className="h-64 flex items-end justify-between px-4 pt-4 relative">
            ...
            {monthlyObservations.map((day: any, idx: number) => {
              const maxVal = Math.max(...monthlyObservations.map((d: any) => d.count)) || 1;
              const barHeight = (day.count / maxVal) * 80; // percent height
              return (
                <div key={idx} className="flex flex-col items-center gap-2 z-10 w-1/5 group">
                  ...
                  <div className="w-8 sm:w-12 bg-slate-100 rounded-t-xl h-44 flex items-end overflow-hidden">
                    <motion.div
                      ...
                      className="w-full bg-gradient-to-t from-blue-600 to-indigo-500 rounded-t-xl group-hover:from-blue-500 group-hover:to-indigo-400 transition-colors"
                    ></motion.div>
                  </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="relative flex items-center justify-center h-48">
            ...
            <div className="absolute text-center">
              <span className="text-2xl font-bold text-slate-900">{categoryChart.reduce((a: any, b: any) => a + b.value, 0) || totals.observations}</span>
          ...
          <div className="mt-6 space-y-2">
            {categoryChart.slice(0, 3).map((item: any, idx: number) => (
            ...
            {categoryChart.length === 0 && (
              <div className="text-xs text-slate-400 text-center py-4">No categories recorded yet.</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm lg:col-span-1 space-y-4">
          ...
          <div className="space-y-3">
            {classrooms.map((cls: any) => {
              ...
              return (
                <div key={cls.id} className="p-3 bg-slate-50 rounded-2xl border border-slate-100 relative group overflow-hidden">
                  <div className="flex items-center justify-between">
                    <div>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm lg:col-span-2 space-y-4">
          ...
          <div className="divide-y divide-slate-50 font-sans">
            ...
            {recentLogs.length === 0 && (
              <p className="text-xs text-slate-400 text-center py-8">No activities recorded in audit logs.</p>
            )}
          </div>
        </div>
      </div>
