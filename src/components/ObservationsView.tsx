...
import { Child, Observation, User } from "../types";
import { motion, AnimatePresence } from "motion/react";
import { Search, Plus, Filter, Sparkles, Trash2, Calendar, AlertTriangle, Eye, RefreshCw, X, MessageSquare, Edit } from "lucide-react";
...
export default function ObservationsView({ user, addToast, setActiveTab, setSelectedObservationId }: ObservationsProps) {
  ...
  const handleTriggerAI = async (obsId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        <div className="relative w-full md:flex-1">
          ...
          <input
            type="text"
            placeholder="Search observations description, student, or teacher..."
            value={searchQuery}
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 px-3 py-2 rounded-lg text-xs font-semibold text-slate-600">
            ...
            <select
              ...
              className="bg-transparent focus:outline-none cursor-pointer max-w-[120px] truncate"
            >
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {filteredObservations.map(obs => {
            ...
            return (
              <motion.div
                key={obs.id}
                layout
                className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-6 relative overflow-hidden"
              >
                {/* Left meta info */}
                ...
                <div className="flex-1 space-y-3.5">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {obs.riskLevel !== "Low" && (
                      <span className={`px-2.5 py-1 text-xs font-semibold border rounded-lg flex items-center gap-1 ${
                        obs.riskLevel === "High" ? "bg-red-50 text-red-700 border-red-100" : "bg-amber-50 text-amber-700 border-amber-100"
                      }`}>
                    ...
                    {obs.status === "Analyzed" ? (
                      <span className="px-2.5 py-1 text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-lg">
                        AI Processed
                      </span>
                    ) : (
                      <span className="px-2.5 py-1 text-xs font-semibold bg-slate-50 text-slate-500 border border-slate-100 rounded-lg">
                        Observation Drafted
                      </span>
                    )}
                  </div>
                ...
                <div className="flex md:flex-col justify-end gap-2.5 shrink-0 pt-4 md:pt-0 md:pl-4 border-t md:border-t-0 md:border-l border-slate-200 items-center justify-center">
                  {user.role !== "Parent" && (
                    <button
                      onClick={(e) => handleTriggerAI(obs.id, e)}
                      disabled={isAnalyzing}
                      className={`w-full md:w-36 flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg font-medium text-xs shadow-sm transition-all ${
                        obs.status === "Analyzed"
                  ...
                  {user.role === "Super Admin" && (
                    <button
                      ...
                      className="p-2 border border-slate-200 text-slate-400 hover:text-red-600 rounded-lg hover:border-red-200 transition-colors bg-white"
                    >
          ...
          {filteredObservations.length === 0 && (
            <div className="py-16 bg-white border border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center text-center px-4">
        ...
        {isAddOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-8">
            ...
            <motion.div
              ...
              className="relative bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden z-10 border border-slate-200"
            >
              ...
              <form onSubmit={handleCreateObservation} className="p-6 space-y-5">
                <div>
                  ...
                  >
                    {children.map(c => (
                      <option key={c.id} value={c.id}>{c.fullName} ({c.classroomName})</option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="block w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/15 focus:border-blue-500 text-sm resize-none leading-relaxed"
                    placeholder="Describe what you observed. Keep it precise, factual, e.g. 'Riya shared blocks during the session but struggled to request water during lunch, resorting to pointing and crying when not understood.'"
                  />
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="px-5 py-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-lg text-xs transition-colors shadow-sm flex items-center gap-1.5"
                  >
                    {submitting ? (
