...
import { User, getUserAvatar, getChildAvatar } from "./types";
...
export default function App() {
  ...
  const [selectedChildId, setSelectedChildId] = useState<string | null>(localStorage.getItem("fc_selected_child_id"));
    ...
    async function initSession() {
      try {
        ...
        if (currentUser) {
          setUser(currentUser);
          if (
            (currentUser.role === "Parent" && currentUser.email === "parent@firstcry.com") ||
  ...
  const handleLoginSuccess = (loggedInUser: User) => {
    ...
    if (
      (loggedInUser.role === "Parent" && loggedInUser.email === "parent@firstcry.com") ||
          ...
          <AnimatePresence>
            {toasts.map((toast) => (
              <motion.div
                ...
                className="bg-white p-4 rounded-xl border border-slate-200 shadow-lg flex gap-3 items-start relative overflow-hidden font-sans"
              >
                {/* Visual Accent bar */}
                <div
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans text-slate-900">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-white border-r border-slate-200 shrink-0 h-screen sticky top-0 no-print">
        ...
        <div className="p-4 border-t border-slate-200">
          <div className="flex items-center gap-3 px-2">
            <img
              src={user.avatar || getUserAvatar(user.gender, user.name, user.role)}
              alt={user.name}
              referrerPolicy="no-referrer"
              className="h-10 w-10 object-cover rounded-md bg-slate-200 border border-slate-200 shadow-sm"
            />
          ...
          {(user.role === "Parent" || user.role === "Teacher") && (
            <button
              onClick={() => setSelectingProfile(true)}
              className="w-full mt-3 py-1.5 border border-slate-200 hover:border-blue-400 hover:bg-blue-50/50 text-blue-600 font-semibold rounded-md text-xs transition-all cursor-pointer flex items-center justify-center gap-1.5"
            >
          ...
          <button
            ...
            className="w-full mt-2 flex items-center justify-center gap-2 px-4 py-2 bg-slate-50 hover:bg-slate-100 text-slate-600 font-semibold border border-slate-200 rounded-md text-xs transition-colors cursor-pointer"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen">
        ...
        <header className="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 sm:px-8 sticky top-0 z-30 no-print">
          ...
          {user && user.role === "Parent" && (
            <div className="flex items-center gap-2 bg-blue-50 text-blue-800 px-3 py-1.5 rounded-lg border border-blue-100 shadow-sm text-xs select-none mx-2 truncate">
              ...
              <button
                ...
                className="text-[10px] font-bold text-blue-600 hover:underline hover:text-blue-800 transition-colors ml-1 border-l border-blue-200 pl-2 cursor-pointer whitespace-nowrap"
              >
                Switch
              </button>
            </div>
          ...
          <div className="flex items-center gap-4 relative">
            ...
            <div className="relative">
              <button
                ...
                className="h-8 w-8 rounded-full border border-slate-200 flex items-center justify-center text-slate-500 hover:bg-slate-50 relative transition-colors cursor-pointer"
              >
                <Bell className="h-4.5 w-4.5" />
                {unreadCount > 0 && (
                  <span className="absolute top-1.5 right-1.5 h-2 w-2 bg-blue-600 rounded-full animate-ping"></span>
                )}
              </button>
                ...
                {notificationsOpen && (
                  <>
                    ...
                    <motion.div
                      ...
                      className="absolute right-0 mt-2 w-80 bg-white border border-slate-200 rounded-lg shadow-lg z-50 overflow-hidden font-sans"
                    >
                      ...
                      <div className="divide-y divide-slate-200 max-h-64 overflow-y-auto text-xs">
                        {notifications.map((notif) => (
                          <div
                            key={notif.id}
                            className={`p-3.5 pr-8 flex gap-2.5 items-start transition-colors relative group ${
                          ...
                          >
                            ...
                            <button
                              ...
                              className="absolute top-3.5 right-2 h-5 w-5 rounded flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-slate-100 transition-colors cursor-pointer"
            ...
            >
              <img
                ...
                alt={user.name}
                referrerPolicy="no-referrer"
                className="h-6 w-6 object-cover rounded border border-slate-100"
              />
        ...
        <main className="flex-1 p-4 sm:p-8 overflow-y-auto">
          <AnimatePresence mode="wait">
            ...
            >
              {activeTab === "dashboard" && (
                <div key={`dashboard-${selectedChildId || ""}`}>
                  <DashboardView
                    user={user}
                    addToast={addToast}
              ...
              {activeTab === "children" && (
                <div key={`children-${selectedChildId || ""}`}>
                  <ChildrenView user={user} addToast={addToast} />
                </div>
              )}
              {activeTab === "observations" && (
                <div key={`observations-${selectedChildId || ""}`}>
                  <ObservationsView
                    user={user}
                    addToast={addToast}
                    setActiveTab={setActiveTab}
                    setSelectedObservationId={setSelectedReportId}
                  />
                </div>
              )}
              {activeTab === "reports" && (
                <div key={`reports-${selectedChildId || ""}`}>
                  <AIHistoryView
                    user={user}
                    addToast={addToast}
              ...
              {activeTab === "attendance" && (
                <div key={`attendance-${selectedChildId || ""}`}>
        ...
        {mobileMenuOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 bottom-0 w-72 bg-white z-50 flex flex-col p-6 overflow-y-auto shadow-2xl lg:hidden border-r border-slate-200"
            >
      ...
      <AnimatePresence>
        {user && user.role === "Parent" && (!selectedChildId || childSelectorOpen) && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4 overflow-y-auto font-sans"
          >
            <motion.div
              ...
              className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col"
            >
              {/* Header */}
              ...
              <div className="p-4 border-b border-slate-100 bg-white flex items-center gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    type="text"
                    placeholder="Search student by name, classroom, or guardian..."
                    value={childSearchQuery}
                    onChange={(e) => setChildSearchQuery(e.target.value)}
                    className="w-full pl-10 pr-4 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all text-slate-700"
                  />
                </div>
                {childSearchQuery && (
                  <button
                    onClick={() => setChildSearchQuery("")}
                    className="text-xs font-semibold text-slate-500 hover:text-slate-700 px-2 py-1 bg-slate-100 rounded hover:bg-slate-200 transition-all"
                  >
                    Clear
                  </button>
                )}
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50">
                {loadingChildren ? (
                  ...
                  <div className="max-w-md mx-auto bg-white p-6 rounded-2xl border border-slate-200 shadow-sm text-center">
                    ...
                    <form onSubmit={async (e) => {
                      e.preventDefault();
                      if (!linkChildName || !linkChildDob) return;
                      setLinkLoading(true);
                      try {
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3.5 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50 text-slate-800 placeholder-slate-400"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          required
                          value={linkChildDob}
                          onChange={(e) => setLinkChildDob(e.target.value)}
                          className="w-full px-3.5 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50 text-slate-700"
                        />
                      </div>
                      <button
                        type="submit"
                        disabled={linkLoading}
                        className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm rounded-xl transition-all shadow-md shadow-blue-500/10 disabled:opacity-50 mt-2 cursor-pointer"
                      >
                              ...
                              {filtered.map((child) => {
                                ...
                                return (
                                  <div
                                    ...
                                    className={`group relative bg-white border rounded-xl p-4 shadow-sm hover:shadow-md transition-all cursor-pointer select-none flex flex-col justify-between ${
                                      isCurrent
                                  ...
                                  >
                                    {isCurrent && (
                                      <span className="absolute top-3 right-3 bg-blue-100 text-blue-800 text-[10px] font-bold px-2 py-0.5 rounded-full">
                                        Active Profile
                                      </span>
                                    )}
                                    ...
                                    <div className="flex items-start gap-3">
                                      <img
                                        ...
                                        alt={child.fullName}
                                        referrerPolicy="no-referrer"
                                        className="h-14 w-14 object-cover rounded-xl border border-slate-100 shadow-sm"
                                      />
                            ...
                            {!showLinkSection ? (
                              <div className="flex justify-center">
                                <button
                                  ...
                                  className="px-4 py-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-semibold text-xs rounded-xl transition-all cursor-pointer shadow-sm flex items-center gap-2"
                                >
                                  <span>🔗</span> Link Another Child's Profile
                                </button>
                              </div>
                            ) : (
                              <div className="max-w-md mx-auto bg-white p-6 rounded-2xl border border-slate-200 shadow-sm text-center relative">
                                <button
                                  type="button"
                                ...
                                <form onSubmit={async (e) => {
                                  ...
                                  <div>
                                    ...
                                    <input
                                      ...
                                      className="w-full px-3 py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50 text-slate-800 placeholder-slate-400"
                                    />
                                  </div>
                                  <div>
                                    ...
                                    <input
                                      ...
                                      value={linkChildDob}
                                      onChange={(e) => setLinkChildDob(e.target.value)}
                                      className="w-full px-3 py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50 text-slate-750"
                                    />
                                  </div>
                                  <button
                                    type="submit"
                                    disabled={linkLoading}
                                    className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs rounded-lg transition-all disabled:opacity-50 mt-1 cursor-pointer"
                                  >
                                    {linkLoading ? "Searching records..." : "Securely Link Profile"}
                                  </button>
                                </form>
              ...
              <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400 font-medium">
                ...
                {selectedChildId ? (
                  <button
                    onClick={() => setChildSelectorOpen(false)}
                    className="px-4 py-1.5 bg-slate-200 text-slate-700 rounded-lg hover:bg-slate-300 transition-all font-semibold cursor-pointer"
                  >
                    Cancel
                  </button>
                ) : (
