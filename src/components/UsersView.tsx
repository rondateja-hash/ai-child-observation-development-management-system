...
import { Search, Plus, Filter, Edit, Trash2, X, Check, Mail, Phone, Lock, Sparkles, UserCheck } from "lucide-react";
...
export default function UsersView({ user, addToast }: UsersProps) {
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {(user.role === "Super Admin" || user.role === "Centre Head" || user.role === "Centre Admin") && (
          <button
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        <div className="relative w-full md:flex-1">
          ...
          <input
            type="text"
            placeholder="Search users directory by name or email address..."
            value={searchQuery}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredUsers.map(u => (
            <div
              key={u.id}
              className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between hover:border-blue-300 hover:shadow-md transition-all duration-300 relative group overflow-hidden"
            >
                ...
                {(user.role === "Super Admin" || user.role === "Centre Head" || user.role === "Centre Admin") && (
                  <div className="flex gap-1.5">
                    <button
                      ...
                      className="p-1.5 bg-slate-50 hover:bg-blue-50 text-slate-600 hover:text-blue-600 border border-slate-200 hover:border-blue-200 rounded-md transition-colors"
                    >
                      <Edit className="h-4 w-4" />
                    </button>
                    <button
                      onClick={() => handleDeleteUser(u.id)}
                      className="p-1.5 bg-slate-50 hover:bg-red-50 text-slate-400 hover:text-red-600 border border-slate-200 hover:border-red-200 rounded-md transition-colors bg-white"
                    >
        ...
        {isAddOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-8">
            ...
            <motion.div
              ...
              className="relative bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden z-10 font-sans border border-slate-200"
            >
              ...
              <form onSubmit={handleFormSubmit} className="p-6 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="block w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/15 focus:border-blue-500 text-sm"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="block w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/15 focus:border-blue-500 text-sm"
                    placeholder="e.g. shalini@firstcry.com"
                  />
                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="block w-full pl-10 pr-3.5 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/15 focus:border-blue-500 text-sm"
                        placeholder="••••••••"
                      />
                    </div>
                  </div>
